...
if [ -n "${SSH_CONNECTION}" ]; then
    CNX=${ALERT1}        # Connected on remote machine, via ssh (good).
elif [[ "${DISPLAY%%:0*}" != "" ]]; then
    CNX=${ALERT4}        # Connected on remote machine, not via ssh (bad).
else
    CNX=${NORMAL}        # Connected on local machine.
fi
...
if [[ ${USER} == "root" ]]; then
    ...
    SU=${NORMAL}         # User is normal (well ... most of us are).
fi
...
function disk_color() {
    ...
    if [ -s "${PWD}" ] ; then
        ...
        echo -en ${DISABLED} # cwd size '0' (like /proc, /sys etc).
    fi
    # perms color (writable, owner, in group)
    if [ ! -w "${PWD}" ] ; then
        echo -en ${On_Red} # No 'write' perms in cwd
    fi
}
...
export HOSTFILE=$HOME/.hosts    # Put a list of remote hosts in
# ~/.hosts
export IGNOREEOF=0
...
alias linode='ssh metaperture@96.126.104.79 -p 121'
alias lnproxy='ssh -D 10080 metaperture@96.126.104.79 -p 121'
